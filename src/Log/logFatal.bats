...
generateLogs() {
  ...
  (
    trap '' EXIT
    Log::logFatal 'fatal' 2>/dev/null
  ) || true
}

function Log::logFatal::debugLevel { #@test
  stub date \
    '* : echo "dateMocked"' \
    '* : echo "dateMocked"'
...
function Log::logFatal::infoLevel { #@test
  stub date \
    '* : echo "dateMocked"' \
    '* : echo "dateMocked"'
...
function Log::logFatal::successLevel { #@test
  stub date \
    '* : echo "dateMocked"' \
    '* : echo "dateMocked"'
...
function Log::logFatal::warningLevel { #@test
  stub date \
    '* : echo "dateMocked"'
  ...
  assert_line --index 0 "dateMocked|  FATAL|fatal"
}

function Log::logFatal::errorLevel { #@test
  stub date \
    '* : echo "dateMocked"'
...
function Log::logFatal::offLevel { #@test
  stub date \
    '* : echo "dateMocked"'
