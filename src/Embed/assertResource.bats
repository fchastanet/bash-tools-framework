...
source "${srcDir}/Embed/getSrcDirsFromOptions.sh"
...
function Embed::assertResource::Empty { #@test
  run Embed::assertResource ""
  assert_failure 1
  assert_output --partial "ERROR   - Invalid embed resource ''. The resource is neither a file, directory nor bash framework function"
}

function Embed::assertResource::InvalidFile { #@test
  ln -s "/dev/null" "${BATS_TEST_TMPDIR}/symbolicLink"
  run Embed::assertResource "${BATS_TEST_TMPDIR}/symbolicLink"
  assert_failure 1
...
function Embed::assertResource::InvalidDirectory { #@test
  run Embed::assertResource "/dev/null"
  assert_failure 1
...
function Embed::assertResource::InvalidBashFrameworkFunction { #@test
  run Embed::assertResource "Embedded::data::François"
  assert_failure 1
...
function Embed::assertResource::File { #@test
  run Embed::assertResource "${BATS_TEST_DIRNAME}/testsData/binaryFile"
  assert_success
  assert_output ""
}

function Embed::assertResource::Directory { #@test
  run Embed::assertResource "${BATS_TEST_DIRNAME}/testsData"
  assert_success
  assert_output ""
}

function Embed::assertResource::BashFrameworkFunctionNotFound { #@test
  local -a _EMBED_COMPILE_ARGUMENTS=(
    --src-dir "${BATS_TEST_DIRNAME}"
  )
  local status=0
  Embed::assertResource "Embed::assertResource" "${BATS_TEST_DIRNAME}" >"${BATS_TEST_TMPDIR}/result" 2>&1 || status=$?
  [[ "${status}" = "2" ]]
  run cat "${BATS_TEST_TMPDIR}/result"
  assert_output --partial "ERROR   - Invalid embed resource 'Embed::assertResource'. The bash framework function is not found in src dirs: ${BATS_TEST_DIRNAME}"
}

function Embed::assertResource::BashFrameworkFunction { #@test
  local -a _EMBED_COMPILE_ARGUMENTS=(
    --src-dir "${srcDir}"
  )
  local status=0
  Embed::assertResource "Embed::assertResource" >"${BATS_TEST_TMPDIR}/result" 2>&1 || status=$?
  [[ "${status}" = "0" ]]
  run cat "${BATS_TEST_TMPDIR}/result"
  assert_output ""
}
