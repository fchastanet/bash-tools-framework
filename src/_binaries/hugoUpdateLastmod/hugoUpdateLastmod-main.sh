...
# Get git date for a file
# @arg $1 = file path
# @arg $2 = command to use (e.g., head, tail) to get first or last commit date
getGitDate() {
  local file="$1"
  local command="$2"
  local timestamp
  ...
  timestamp=$(git log --follow --format="%aI" --reverse -- "${file}" | "${command}" -1)
...
processMigrationMode() {
  ...
  if ! hasFrontmatterField "${frontmatter}" "date"; then
    if hasFrontmatterField "${frontmatter}" "creationDate"; then
      ...
      # Get from git
      dateValue=$(getGitDate "${file}" "head")
  ...
  if ! hasFrontmatterField "${frontmatter}" "lastmod"; then
    if hasFrontmatterField "${frontmatter}" "lastUpdated"; then
      ...
      # Get from git
      lastmodValue=$(getGitDate "${file}" "tail")
