...
function Options::generateCommand::case11 { #@test
  local optionFile
  local status=0
  helpFunction() {
    echo "help generated by function"
  }
  export -f helpFunction
  longHelpFunction() {
    echo "long help generated by function"
  }
  export -f longHelpFunction
  local optionFile
  optionFile="$(Options::generateOption --variable-type String --help "file" \
    --variable-name "file" --alt "--file" --alt "-f")" || return 1
  Options::sourceFunction "${optionFile}"

  Options::generateCommand --help "helpFunction" \
    --long-description "longHelpFunction" "${optionFile}" >"${BATS_TEST_TMPDIR}/result" 2>&1 || status=$?

  testCommand "generateCommand.case11.sh" "Options::command"
}

function Options::generateCommand::case11::help { #@test
  source "${BATS_TEST_DIRNAME}/testsData/generateCommand.case11.sh"
  helpFunction() {
    echo "help generated by function"
  }
  export -f helpFunction
  ...
  export -f longHelpFunction
  run Options::command help
  checkCommandResult "generateCommand.case11.expected.help"
}
